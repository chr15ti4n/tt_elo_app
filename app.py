...
# Zero-sum group ELO for Rundlauf: winner=1.0, second=0.5, others=0.0
# Ensures that the sum of rating changes across all participants is 0 (up to rounding; we correct drift).
def _calc_round_group_deltas(ratings: dict[str, float], winner_id: str, fin1_id: str, fin2_id: str, k: int = 48) -> dict[str, int]:
    import math
    # Total score mass to distribute: 1.0 (winner) + 0.5 (second) + 0 for others
    S_total = 1.5
    # Softmax (base 10) over current ratings → probabilities that sum to 1
    pow_map = {pid: 10 ** (float(r) / 400.0) for pid, r in ratings.items()}
    denom = sum(pow_map.values()) or 1.0
    # Expected share scaled to the total score mass
    exp = {pid: S_total * (pow_map[pid] / denom) for pid in ratings.keys()}
    # Observed outcome per player
    obs = {}
    for pid in ratings.keys():
        if pid == str(winner_id):
            obs[pid] = 1.0
        elif pid == str(fin1_id) or pid == str(fin2_id):
            # If a finalist that is not the winner → second place
            if pid != str(winner_id):
                obs[pid] = 0.5
            else:
                # already handled winner
                obs[pid] = 1.0
        else:
            obs[pid] = 0.0
    # Raw integer deltas using common K
    deltas = {pid: int(round(k * (obs[pid] - exp[pid]))) for pid in ratings.keys()}
    # Correct rounding drift to keep exact zero-sum
    drift = sum(deltas.values())
    if drift != 0:
        # Assign drift to the winner (any deterministic rule is fine)
        w = str(winner_id)
        deltas[w] = deltas.get(w, 0) - drift
    return deltas

# Hinweis: Die folgende Einzel-Funktion wird aktuell nicht mehr für Updates genutzt (ersetzt durch _calc_round_group_deltas).
...
def update_round_after_confirm_id(participant_ids: list[str], fin1_id: str, fin2_id: str, winner_id: str, k: int = 48) -> None:
    ...
    # 2) Zero-sum Deltas gemeinschaftlich berechnen (statt einzelner Erwartungswert gegen Durchschnitt)
    ratings = {pid: float(rec.get("r_elo", 1200)) for pid, rec in current.items()}
    deltas = _calc_round_group_deltas(ratings, winner_id=winner_id, fin1_id=fin1_id, fin2_id=fin2_id, k=k)

    # 3) Für jeden Spieler neues r_elo + Stats + g_elo
    for pid, rec in current.items():
        delta = int(deltas.get(pid, 0))
        new_r = int(rec.get("r_elo", 1200)) + delta
        # Sieges-/Zweit-/Niederlagen-Logik
        if pid == str(winner_id):
            inc = {"r_siege": 1, "r_zweite": 0, "r_niederlagen": 0}
        elif pid in (str(fin1_id), str(fin2_id)):
            # Finalist und nicht Gewinner → zweiter Platz
            if pid != str(winner_id):
